...
public abstract class StructSupport implements Map, Struct {
	...
	public PageException invalidKey(Config config, Struct sct, Key key, String in) {
		...
		while (it.hasNext()) {
			k = it.next();
			if (k.equals(key)) return engine.getExceptionUtil()
					.createExpressionException("the value from key [" + key.getString() + "] " + appendix + " is NULL, which is the same as not existing in CFML");
		}
	...
	public Object callWithNamedValues(PageContext pc, Key methodName, Struct args) throws PageException {
		...
		throw CFMLEngineFactory.getInstance().getExceptionUtil().createApplicationException("no function with name " + methodName + " found!");
		// TODO return MemberUtil.callWithNamedValues(pc,this,methodName,args, CFTypes.TYPE_STRUCT,
		// "struct");
	}
	...
	/*
	 * TODO public boolean equals(Object obj){ if(!(obj instanceof Collection)) return false; return
	...
	private RuntimeException notSupported() {
		return CFMLEngineFactory.getInstance().getExceptionUtil()
				.createPageRuntimeException(CFMLEngineFactory.getInstance().getExceptionUtil().createApplicationException("this method is not supported!"));
	}
}